...
@PIPELINES.register_module()
class LoadPointsFromFileVID(object):
    """Load Points From File.
    ...
    def __call__(self, results):
        ...
        """
        results['points'] = []
        for i in range(len(results['pts_filename'])):
            pts_filename = results['pts_filename'][i]
            points = self._load_points(pts_filename)
            points = points.reshape(-1, self.load_dim)
            points = points[:, self.use_dim]
            attribute_dims = None
...
@PIPELINES.register_module()
class LoadPointsFromMultiSweepsVID(object):
    """Load points from multiple sweeps.
    ...
    def __call__(self, results):
        ...
        for i in range(len(results['points'])):
            ...
            points.tensor[:, 4] = 0
            sweep_points_list = [points]
            ts = results['timestamp'][i]
            if self.pad_empty_sweeps and len(results['sweeps'][i]) == 0:
                for i in range(self.sweeps_num):
                    if self.remove_close:
                        sweep_points_list.append(self._remove_close(points))
                    else:
                ...
                if len(results['sweeps'][i]) <= self.sweeps_num:
                    choices = np.arange(len(results['sweeps'][i]))
                elif self.test_mode:
                    choices = np.arange(self.sweeps_num)
                else:
                    choices = np.random.choice(
                        len(results['sweeps'][i]), self.sweeps_num, replace=False)
                for idx in choices:
                    sweep = results['sweeps'][i][idx]
                    points_sweep = self._load_points(sweep['data_path'])
                    points_sweep = np.copy(points_sweep).reshape(-1, self.load_dim)
                    if self.remove_close:
                        points_sweep = self._remove_close(points_sweep)
...
@PIPELINES.register_module()
class LoadAnnotations3DVID(LoadAnnotations):
    """Load Annotations3D.
    ...
    def _load_labels_3d(self, results):
        ...
        for i in range(len(results['ann_info'])):
            results['gt_labels_3d'].append(results['ann_info'][i]['gt_labels_3d'])
...
@PIPELINES.register_module()
class LoadMultiViewImageFromFilesVID(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __call__(self, results):
        ...
        for i in range(len(results['img_filename'])):
            ...
            # unravel to list, see `DefaultFormatBundle` in formating.py
            # which will transpose each image separately and then stack into array
            results['img'].append([img[..., i] for i in range(img.shape[-1])])
            ...
            # results['scale_factor'] = [1.0, 1.0]
            num_channels = 1 if len(img.shape) < 3 else img.shape[2]
            results['img_norm_cfg'].append(dict(
                mean=np.zeros(num_channels, dtype=np.float32),
                std=np.ones(num_channels, dtype=np.float32),
                to_rgb=False))
            results['img_fields'] = ['img']
...
class MyResizeVID(object):
    ...
    def _resize_img(self, results):
        ...
        for i in range(len(results['sample_idx'])):
            imgs = results['img'][i]
            results['img'][i] = [imgs[i] for i in range(len(imgs))]
            for key in results.get('img_fields', ['img']):
                for idx in range(len(results['img'][i])):
                    if self.keep_ratio:
                        img, scale_factor = mmcv.imrescale(
                            results[key][i][idx],
                            results['scale'],
                        ...
                        img, w_scale, h_scale = mmcv.imresize(
                            ...
                            return_scale=True,
                            backend=self.backend)
                    results[key][i][idx] = img

                scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                        dtype=np.float32)
                results['img_shape'][i] = img.shape
                # in case that there is no padding
                results['pad_shape'][i] = img.shape
                results['scale_factor'].append(scale_factor)
                results['keep_ratio'].append(self.keep_ratio)
...
class MyNormalizeVID(object):
    ...
    def __call__(self, results):
        ...
        for i in range(len(results['sample_idx'])):
            for key in results.get('img_fields', ['img']):
                for idx in range(len(results['img'][i])):
                    results[key][i][idx] = mmcv.imnormalize(results[key][i][idx], self.mean, self.std, self.to_rgb)
            results['img_norm_cfg'].append(dict(
                mean=self.mean, std=self.std, to_rgb=self.to_rgb))
    ...
    def _pad_img(self, results):
        ...
        for i in range(len(results['sample_idx'])):
            for key in results.get('img_fields', ['img']):
                if self.size is not None:
                    padded_img = mmcv.impad(
                        results[key][i], shape=self.size, pad_val=self.pad_val)
                elif self.size_divisor is not None:
                    for idx in range(len(results[key][i])):
                        padded_img = mmcv.impad_to_multiple(
                            results[key][i][idx], self.size_divisor, pad_val=self.pad_val)
                        results[key][i][idx] = padded_img
            results['pad_shape'][i] = padded_img.shape
            results['pad_fixed_size'].append(self.size)
            results['pad_size_divisor'].append(self.size_divisor)
