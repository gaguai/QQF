...
class _ObjectSampleVID(object):
    ...
    def __call__(self, input_dict):
        ...
        for i in range(len(input_dict['gt_bboxes_3d'])):
            ...
            crop_img_list = [[np.zeros((1,1,1))] * num_gt for _ in range(len(cam_orders))]
            if self.with_info:
                sample_record = data_info.get('sample', input_dict['sample_idx'][i])
                pointsensor_token = sample_record['data']['LIDAR_TOP']
            if self.with_info and num_gt:
                # Transform points
                sample_coords = input_dict["gt_bboxes_3d"][i].corners
                #  sample_coords = box_np_ops.rbbox3d_to_corners(gt_bboxes_3d.tensor.cpu().numpy())
                #  crop_img_list = [[] for _ in range(len(sample_coords))]
                #  crop_img_list = [[np.zeros((1,1,1))] * len(sample_coords) for _ in range(len(cam_orders))]
                # Crop images from raw images
                for _idx, _key in enumerate(cam_orders):
                    ...
                    bbox = np.concatenate([minxy, maxxy], axis=-1)
                    #  bbox = (bbox * res['image_scale']).astype(np.int32)
                    bbox[:,0::2] = np.clip(bbox[:,0::2], a_min=0, a_max=input_dict['img_shape'][i][1]-1)
                    bbox[:,1::2] = np.clip(bbox[:,1::2], a_min=0, a_max=input_dict['img_shape'][i][0]-1)
                    cam_mask = (bbox[:,2]-bbox[:,0])*(bbox[:,3]-bbox[:,1])>0
                    if cam_mask.sum() == 0:
                        continue
                    cam_count = cam_count[cam_mask]
        ...
        if self.sample_2d:
            img = input_dict['img']
            gt_bboxes_3d_np = [input_dict['gt_bboxes_3d'][i].tensor.numpy() for i in range(len(input_dict['gt_bboxes_3d']))]
            gt_bboxes_3d = [input_dict['gt_bboxes_3d'][i] for i in range(len(input_dict['gt_bboxes_3d']))]
            gt_labels_3d = [input_dict['gt_labels_3d'][i] for i in range(len(input_dict['gt_labels_3d']))]
            # Assume for now 3D & 2D bboxes are the same
            #  sampled_dict = self.db_sampler.sample_all(
                #  gt_bboxes_3d.tensor.numpy(),
                #  gt_labels_3d,
        ...
        if sampled_dict is not None:
            ...
            for i in range(len(sampled_gt_bboxes_3d)):
                gt_labels_3d_update.append(np.concatenate([gt_labels_3d[i], sampled_gt_labels[i]],
                                          axis=0).astype(np.long))
                # for changed pkl
                gt_bboxes_3d_update.append(gt_bboxes_3d[i].new_box(
                    np.concatenate([gt_bboxes_3d[i].tensor.numpy(), sampled_gt_bboxes_3d[i]])))
                points[i] = self.remove_points_in_boxes(points[i], sampled_gt_bboxes_3d[i])
                # check the points dimension
                points[i] = points[i].cat([sampled_points[i], points[i]])

            gt_bboxes_3d = copy.deepcopy(gt_bboxes_3d_update)
            gt_labels_3d = copy.deepcopy(gt_labels_3d_update)
            ...
            if self.with_info:
                for i in range(len(gt_bboxes_3d)):
                    ...
                    sample_crops = []
                    for cam_order in range(len(cam_orders)):
                        sample_crops.append(crop_img_list_seq[i][cam_order] + sampled_dict['img_crops'][i])
                    if not self.keep_raw:
                        points_coords = points[i][:,:4].copy()
                        points_coords[:,-1] = 1
                        point_keep_mask = np.ones(len(points_coords)).astype(np.bool)
                    # Paste image according to sorted strategy
                    for _idx, _key in enumerate(cam_orders):
                        ...
                        cam_count = cam_count[cam_mask][paste_order]
                        bbox = bbox[cam_mask][paste_order]
                        paste_mask = -255 * np.ones(input_dict['img_shape'][i][:2], dtype=np.int64)
                        fg_mask = np.zeros(input_dict['img_shape'][i][:2], dtype=np.int64)
                        for _count, _box in zip(cam_count, bbox.astype(np.int)):
                            img_crop = sample_crops[_idx][_count]
                            #  img_crop = sample_crops[_count]
                        ...
                        if not self.keep_raw:
                            ...
                            point_img = point_img.transpose()[...,:2]
                            #  point_img = (point_img * res['image_scale']).astype(np.int64)
                            cam_mask = (point_img[:,0] > 0) & (point_img[:,0] < input_dict['img_shape'][i][1]) & \
                                    (point_img[:,1] > 0) & (point_img[:,1] < input_dict['img_shape'][i][0]) & cam_mask
                            point_img = point_img[cam_mask]
                            new_mask = paste_mask[point_img[:,1], point_img[:,0]]==(points_idx[cam_mask]+raw_gt_box_num)
                            raw_fg = (fg_mask == 1) & (paste_mask >= 0) & (paste_mask < raw_gt_box_num)
                            raw_bg = (fg_mask == 0) & (paste_mask < 0)
                    ...
                    # Replace the original images
                    input_dict['img'][i] = cam_images
                    # remove overlaped LIDAR points
                    if not self.keep_raw:
                        points[i] = points[i][point_keep_mask]
            if self.sample_2d and False:
                sampled_gt_bboxes_2d = sampled_dict['gt_bboxes_2d']
                gt_bboxes_2d = np.concatenate(
                    [gt_bboxes_2d, sampled_gt_bboxes_2d]).astype(np.float32)
        ...
        #     bev_cur = nuscene_vis(input_dict['points'][0].tensor.cpu().numpy(),
        #             input_dict['gt_bboxes_3d'][0].tensor.numpy())
        #     bev_prev = nuscene_vis(input_dict['points'][1].tensor.cpu().numpy(),
        #             input_dict['gt_bboxes_3d'][1].tensor.numpy())
        #     front_cur = input_dict['img'][0][1]
        #     front_prev = input_dict['img'][1][1]
                    
        #     cv2.imwrite('demo_bev_cur.png', bev_cur)
        #     cv2.imwrite('demo_bev_prev.png', bev_prev)
        #     cv2.imwrite('demo_cam_front_cur.png', front_cur)
        #     cv2.imwrite('demo_cam_front_prev.png', front_prev)
...
@PIPELINES.register_module()
class GlobalRotScaleTransVID(object):
    """Apply global rotation, scaling and translation to a 3D scene.
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        for i in range(len(input_dict['sample_idx'])):
            input_dict['points'][i].translate(trans_factor)
            input_dict['pcd_trans'] = trans_factor
            for key in input_dict['bbox3d_fields']:
                input_dict[key][i].translate(trans_factor)
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        for i in range(len(input_dict['sample_idx'])):
            for key in input_dict['bbox3d_fields']:
                if len(input_dict[key][i].tensor) != 0:
                    points, rot_mat_T = input_dict[key][i].rotate(
                        noise_rotation, input_dict['points'][i])
                    input_dict['points'][i] = points
                    input_dict['pcd_rotation'] = rot_mat_T
        # input_dict['points_instance'].rotate(noise_rotation)
    ...
    def __call__(self, input_dict):
        ...
        #     cv2.imwrite('demo_bev_cur_rot.png', bev_cur)
        #     cv2.imwrite('demo_bev_prev_rot.png', bev_prev)
...
@PIPELINES.register_module()
class RandomFlip3DVID(RandomFlip):
    """Flip the points & bbox.
    ...
    def __init__(self,
                 ...
                 flip_ratio_bev_vertical=0.0,
                 **kwargs):
        super(RandomFlip3DVID, self).__init__(
            flip_ratio=flip_ratio_bev_horizontal, **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        for i in range(len(input_dict['sample_idx'])):
            ...
            assert len(input_dict['bbox3d_fields']) == 1
            for key in input_dict['bbox3d_fields']:
                input_dict['points'][i] = input_dict[key][i].flip(
                    direction, points=input_dict['points'][i])
    ...
    def __call__(self, input_dict):
        ...
        #     cv2.imwrite('demo_bev_cur_flip.png', bev_cur)
        #     cv2.imwrite('demo_bev_prev_flip.png', bev_prev)
...
@PIPELINES.register_module()
class ObjectRangeFilterVID(object):
    """Filter objects by the range.
    ...
    def __call__(self, input_dict):
        ...
        for i in range(len(input_dict['sample_idx'])):
            ...
            # limit rad to [-pi, pi]
            gt_bboxes_3d.limit_yaw(offset=0.5, period=2 * np.pi)
            input_dict['gt_bboxes_3d'][i] = gt_bboxes_3d
            input_dict['gt_labels_3d'][i] = gt_labels_3d
...
@PIPELINES.register_module()
class PointsRangeFilterVID(object):
    """Filter points by the range.
    ...
    def __call__(self, input_dict):
        ...
        for i in range(len(input_dict['sample_idx'])):
            ...
            points_mask = points.in_range_3d(self.pcd_range)
            clean_points = points[points_mask]
            input_dict['points'][i] = clean_points
...
@PIPELINES.register_module()
class ObjectNameFilterVID(object):
    """Filter GT objects by their names.
    ...
    def __call__(self, input_dict):
        ...
        for i in range(len(input_dict['sample_idx'])):
            ...
            gt_bboxes_mask = np.array([n in self.labels for n in gt_labels_3d],
                                        dtype=np.bool_)
            input_dict['gt_bboxes_3d'][i] = input_dict['gt_bboxes_3d'][i][gt_bboxes_mask]
            input_dict['gt_labels_3d'][i] = input_dict['gt_labels_3d'][i][gt_bboxes_mask]
        # if True:
        #     from mmdet3d.utils.simplevis import nuscene_vis
        #     bev_cur = nuscene_vis(input_dict['points'][0].tensor.cpu().numpy(),
        #             input_dict['gt_bboxes_3d'][0].tensor.numpy())
        ...
        #     cv2.imwrite('demo_bev_cur_filter.png', bev_cur)
        #     cv2.imwrite('demo_bev_prev_filter.png', bev_prev)
