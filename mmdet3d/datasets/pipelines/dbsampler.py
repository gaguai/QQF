...
class BatchSampler:
    ...
    def sample_vid(self, num):
        indices = self._sample(num)
        return [self._sampled_list[i] for i in indices], indices
...
@OBJECTSAMPLERS.register_module()
class DataBaseTemporalSampler(object):
    """Class for sampling data from the ground truth database.
    ...
    def __init__(self,
        ...
        db_infos = [mmcv.load(info_path_list[i]) for i in range(len(info_path_list))]
        # filter database infos
        from mmdet3d.utils import get_root_logger
        logger = get_root_logger()
        for k, v in db_infos[0].items():
            logger.info(f'load {len(v)} {k} database infos')
        for prep_func, val in prepare.items():
            db_infos = getattr(self, prep_func)(db_infos, val)
        logger.info('After filter database:')
        ...
        self.group_db_infos = self.db_infos[0]  # just use db_infos
        self.sample_classes = []
        self.sample_max_nums = []
        for group_info in self.sample_groups:
            self.sample_classes += list(group_info.keys())
    ...
    def filter_by_difficulty(db_infos, removed_difficulty):
        ...
        Returns:
            dict: Info of database after filtering.
        """
        seq_len = len(db_infos)
        ##############  seq_len 고려안함  ########################
        new_db_infos = [{} for _ in range(seq_len)]
        for key, dinfos in db_infos[0].items():
            new_db_infos[0][key] = []
            new_db_infos[1][key] = []

            for idx, info in enumerate(dinfos):
                if info['difficulty'] not in removed_difficulty and \
                 db_infos[1][key][idx]['difficulty'] not in removed_difficulty:
                
                    new_db_infos[0][key].append(info)
                    new_db_infos[1][key].append(db_infos[1][key][idx])
    ...
    def filter_by_min_points(db_infos, min_gt_points_dict):
        ...
        for name, min_num in min_gt_points_dict.items():
            ...
            if min_num > 0:
                filtered_infos = []
                filtered_prev_infos = []
                for idx, info in enumerate(db_infos[0][name]):
                    if info['num_points_in_gt'] >= min_num and \
                    db_infos[1][name][idx]['num_points_in_gt'] >= min_num:
                        filtered_infos.append(info)
                        filtered_prev_infos.append(db_infos[1][name][idx])
                db_infos[0][name] = filtered_infos
                db_infos[1][name] = filtered_prev_infos
        return db_infos
    ...
    def sample_all(self, gt_bboxes, gt_labels, img=None, data_info=None, sample=None):
        ...
        for class_name, max_sample_num in zip(self.sample_classes,
            ...
            #                   np.sum([n == class_name for n in gt_names]))
            sampled_num = int(max_sample_num -
                              np.sum([n == class_label for n in gt_labels[0]]))
            sampled_num = np.round(self.rate * sampled_num).astype(np.int64)
            sampled_num_dict[class_name] = sampled_num
            sample_num_per_class.append(sampled_num)
        ...
        for class_name, sampled_num in zip(self.sample_classes,
                                           sample_num_per_class):
            if sampled_num > 0:
                sampled_cls_list = self.sample_class_v2(class_name, 
                                                        sampled_num,
                                                        avoid_coll_boxes)
                assert type(sampled_cls_list) == list
                assert len(sampled_cls_list) == seq_len
                
                if seq_len == 2: 
                    sampled_cur = sampled_cls_list[0]
                    sampled_prev = sampled_cls_list[1]

                    sampled[0] += sampled_cur
                    sampled[1] += sampled_prev
                    
                    if len(sampled_cur) > 0:
                        sampled_gt_box = [[] for t in range(seq_len)]
                        
                        if len(sampled_cur) == 1:
                            sampled_gt_box[0] = sampled_cur[0]["box3d_lidar"][np.newaxis, ...]
                            sampled_gt_box[1] = sampled_prev[0]["box3d_lidar"][np.newaxis, ...]
                        else:
                            sampled_gt_box[0] = np.stack([s["box3d_lidar"] for s in sampled_cur], axis=0)
                            sampled_gt_box[1] = np.stack([s["box3d_lidar"] for s in sampled_prev], axis=0)
                    
                    if len(sampled_cur) > 0:
                        for t in range(seq_len):
                            sampled_gt_bboxes[t] += [sampled_gt_box[t]]

                        avoid_coll_boxes = [np.concatenate([avoid_coll_box, sampled_gt_box[t]], axis=0) for t, avoid_coll_box in enumerate(avoid_coll_boxes)]

        ret = None
        if len(sampled[0]) > 0:
            sampled_gt_bboxes_list = []
            s_points_list = []
            gt_labels_list = []
            crop_imgs_list = []

            for i in range(seq_len):
                sampled_gt_bboxes_list.append(np.concatenate(sampled_gt_bboxes[i], axis=0))
                # sample_coords = box_np_ops.rbbox3d_to_corners(sampled_gt_bboxes_list[i])
                sample_coords = np.array(sample[i].new_box(sampled_gt_bboxes_list[i]).corners)
                # center = sampled_gt_bboxes[:, 0:3]
                ...
                crop_imgs_seq = []
                count = 0
                for _idx, info in enumerate(sampled[i]):
                    file_path = os.path.join(
                        self.data_root,
                        info['path']) if self.data_root else info['path']
                    results = dict(pts_filename=file_path)
                    ...
                    if data_info is not None:
                        ...
                        for c_idx, _key in enumerate(cam_orders):
                            ...
                            crop_img = cam_img[bbox[1]:bbox[3], bbox[0]:bbox[2]]
                            break
                        crop_imgs_seq.append(crop_img)
                
                gt_labels = np.array([self.cat2label[s['name']] for s in sampled[i]],
                                        dtype=np.long)
                gt_labels_list.append(gt_labels)
                crop_imgs_list.append(crop_imgs_seq)
                s_points_list.append(s_points_temp_list[0].cat(s_points_temp_list))
            group_ids = [np.arange(gt_bboxes[i].shape[0], gt_bboxes[i].shape[0]+len(sampled[i])) for i in range(len(gt_bboxes))]
            if 'token' in sampled[0]:
                sampled_gt_bboxes[:, 2] -= sampled_gt_bboxes[:, 5] / 2
            ret = {
                'gt_labels_3d':
    ...
    def sample_class_v2(self, name, num, gt_bboxes):
        ...
        sampled, sample_idx = self.sampler_dict[name].sample_vid(num)
        sampled = copy.deepcopy(sampled)
        # num_gt = gt_bboxes[0].shape[0]
        num_sampled = len(sampled)
        ...
        for i in range(len(gt_bboxes)):
            if i==0:
                continue
            temp_sampled = [self.db_infos[i][name][idx] for idx in sample_idx]
        ...
        if False and 'token' in sampled[0].keys():
            ...
            for i in range(len(sampled)):
                sampled[i]['box3d_lidar'] = sampled[i]['box3d_lidar'][[0, 1, 2, 3, 4, 5, 8, 6, 7]]
        #################################################################
        valid_samples_list = [[] for _ in range(len(gt_bboxes))]
        valid_samples_mask = np.zeros([len(gt_bboxes), len(sampled)])

        for gt_idx, gt_boxes in enumerate(gt_bboxes):
