...
@MIDDLE_ENCODERS.register_module()
class SparseEncoderTempFusion(nn.Module):
    r"""Sparse encoder for SECOND and Part-A2.
    ...
    def forward(self,
        ...
        # breakpoint()
        seq_len = len(img_seq)
        coors_seq = [coors.int() for coors in coors_seq]
        input_sp_tensor_seq = [spconv.SparseConvTensor(voxel_features_seq[i], coors_seq[i],
                                                  self.sparse_shape,
                                                  batch_size) for i in range(seq_len)]
        x_seq = [self.conv_input(input_sp_tensor) for input_sp_tensor in input_sp_tensor_seq]

        encode_features_seq, lidar_features_seq = [], []
        for idx, encoder_layer in enumerate(self.encoder_layers):
            x_seq = [encoder_layer(x) for x in x_seq]
            if ret_lidar_features:
                lidar_features_seq.append(x_seq)
            if self.fusion_pos is not None and idx in self.fusion_pos:
                f_feats = self.fusion_layer(img_feats_seq, x_seq,
                                            img_metas_seq, img_seq,
                                            gt_bboxes_3d)
                x = replace_feature(x, f_feats)
            encode_features_seq.append(x_seq)
