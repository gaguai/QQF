...
class TempTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu",
                 query_posembed=None, cross_posembed=None, cross_only=False):
        super().__init__()
        self.cross_only = cross_only
        # if not self.cross_only:
        #     self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
        ...
        self.q_query_posembed = query_posembed
        self.k_query_posembed = query_posembed
        self.cross_posembed = cross_posembed

    def with_pos_embed(self, tensor, pos_embed):
        return tensor if pos_embed is None else tensor + pos_embed

    def forward(self, obj_query_list, feat_list, 
                obj_query_pos_list, feat_pos_list, 
                seq, attn_mask=None):
        """
        :param obj_query_list: B C Pq
        :param feat_list: B C Pk
        :param obj_query_pos_list: B Pq 3/6
        :param feat_pos_list: B Pk 3/6
        ...
        """
        # NxCxP to PxNxC
        seq_len = len(obj_query_list)
        q_idx = seq_len - seq -1
        if seq == 0:
            k_idx = 0
        else:
            k_idx = seq_len - seq 
        if self.q_query_posembed is not None:
            q_query_pos_embed = self.q_query_posembed(obj_query_pos_list[q_idx]).permute(2, 0, 1)
        else:
            q_query_pos_embed = None
        if self.k_query_posembed is not None:
            k_query_pos_embed = self.k_query_posembed(obj_query_pos_list[k_idx]).permute(2, 0, 1)
        else:
        ...
        if not self.cross_only:
            # q = k = v = self.with_pos_embed(query, query_pos_embed)
            q = self.with_pos_embed(q_query, q_query_pos_embed)
            k = v = self.with_pos_embed(k_query, k_query_pos_embed)
            query2 = self.multihead_query_attn(q, k, value=v)[0]
            query = q_query + self.dropout1(query2)
            query = self.norm1(query)

        query2 = self.multihead_attn(query=self.with_pos_embed(query, q_query_pos_embed),
                                     key=self.with_pos_embed(feat, feat_pos_embed),
                                     value=self.with_pos_embed(feat, feat_pos_embed), attn_mask=attn_mask)[0]
        query = query + self.dropout2(query2)
        query = self.norm2(query)
...
class TempTransFusionHead(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(TempTransFusionHead, self).__init__()
        ...
        for i in range(self.num_decoder_layers):
            ...
            for j in range(self.num_seq):
                self.decoder[i].append(
                    TempTransformerDecoderLayer(
                        hidden_channel, num_heads, ffn_channel, dropout, activation,
                        query_posembed=PositionEmbeddingLearned(2, hidden_channel),
        ...
        for i in range(self.num_decoder_layers):
            self.prediction_heads.append(nn.ModuleList())
            for j in range(self.num_seq):
                heads = copy.deepcopy(common_heads)
                heads.update(dict(heatmap=(self.num_classes, num_heatmap_convs)))
                self.prediction_heads[i].append(FFN(hidden_channel, heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias))
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        """
        query_feat_seq, lidar_feat_flatten_seq, query_pos_seq, bev_pos_seq,heatmap_seq, dense_heatmap_seq =\
        [], [], [], [], [], []
        for seq in range(len(inputs)):
            batch_size = inputs[seq][0].shape[0]
            lidar_feat = self.shared_conv(inputs[seq][0])
            ...
            query_feat_seq.append(query_feat)
            lidar_feat_flatten_seq.append(lidar_feat_flatten)
            query_pos_seq.append(query_pos)
            bev_pos_seq.append(bev_pos)
            heatmap_seq.append(heatmap)
            dense_heatmap_seq.append(dense_heatmap)
        ...
        for i in range(self.num_decoder_layers):
            for seq in range(self.num_seq):
                ...
                query_feat, q_idx = self.decoder[i][seq](query_feat_seq, lidar_feat_flatten_seq, query_pos_seq, bev_pos_seq, seq)

                query_feat_seq[q_idx] = query_feat

                # Prediction
                res_layer = self.prediction_heads[i][seq](query_feat)
                res_layer['center'] = res_layer['center'] + query_pos_seq[q_idx].permute(0, 2, 1)
                first_res_layer = res_layer
                if not self.fuse_img:
                    ret_dicts.append(res_layer)

                # for next level positional embedding
                query_pos_seq[q_idx] = res_layer['center'].detach().clone().permute(0, 2, 1)
...
#####################################질문####################################
        ...
        new_res = {}
        for key in ret_dicts[1].keys():
            if key not in ['dense_heatmap', 'dense_heatmap_old', 'query_heatmap_score']:
                new_res[key] = torch.cat([ret_dict[key] for idx, ret_dict in enumerate(ret_dicts) if (idx+1) % self.num_seq==0], dim=-1)
            else:
