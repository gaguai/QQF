...
class BEVGated_half(nn.Module):
    def __init__(self,
                 in_channels=[512],
                 seq_len=2):
        super(BEVGated_half, self).__init__()
    ...
    def forward(self, inputs):
        ...
        for num_in in range(len(self.in_channels)):
            for seq in range(self.seq_len):
                if seq == 0: # current frame
                    continue
                else:
                    feats.append([inputs[0][num_in],inputs[seq][num_in]])
        
        for sigmoid in self.sigmoid:
            feat_gated_all = 0
            channel = feats[k*(self.seq_len-1)][0].shape[1]

            for seq in range(self.seq_len-1):
                feat = torch.cat(feats[k*(self.seq_len-1)+seq],1)
                gating_weight1 = sigmoid(self.conv_gating[k*(self.seq_len-1)+seq](feat))
                gating_weight2 = 1. - gating_weight1

                feat_gated = feat[:,0:channel,:,:]*gating_weight1 + feat[:,channel:channel*2,:,:]*gating_weight2
                
                feat_gated_all += feat_gated
                
            x_temp.append(feat_gated_all)
            k+=1
...
class GatedTransFusionHead(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(GatedTransFusionHead, self).__init__()
        self.temporal_gate = BEVGated_half(gated_in_channels, gated_seq_len)
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        """
        inputs = self.temporal_gate(inputs)[0]
